...
const NestedList = () => {
  ...
  const toggleVcenter = (vIdx) => {
    setOpenStates(
      openStates.map((vcenter, idx) =>
        idx === vIdx ? { ...vcenter, vcenter: !vcenter.vcenter } : vcenter
      )
    );
  };
  ...
  const toggleDatacenter = (vIdx, dIdx) => {
    setOpenStates(
      openStates.map((vcenter, vi) =>
        vi === vIdx
          ? {
              ...vcenter,
              datacenters: vcenter.datacenters.map((datacenter, di) =>
                di === dIdx
          ...
          ? {
              ...
              datacenters: vcenter.datacenters.map((datacenter, di) =>
                di === dIdx
                  ? {
                      ...datacenter,
                      clusters: datacenter.clusters.map((cluster, ci) =>
                        ci === cIdx
                  ...
                  ? {
                      ...
                      clusters: datacenter.clusters.map((cluster, ci) =>
                        ci === cIdx
                          ? {
                              ...cluster,
                              hosts: cluster.hosts.map((host, hi) =>
                                hi === hIdx ? !host : host
                              ),
                            }
                          : cluster
  ...
  return (
    <List>
      <Typography variant="h5" sx={{ textAlign: "left", mt: 2, mb: 4 }}>
        VMware vCenter Server List
      </Typography>
      {jsonData.map((vcenter, vIdx) => (
        <React.Fragment key={vIdx}>
          ...
          >
            ...
            <ListItemText
              primary={
                <Link href={vcenter.url} target="_blank" rel="noopener">
                  {vcenter.name}
                </Link>
              }
            />
            ...
            <List component="div" disablePadding>
              {vcenter.datacenters.map((datacenter, dIdx) => (
                <React.Fragment key={dIdx}>
                  <ListItem
                    button
                    onClick={() => toggleDatacenter(vIdx, dIdx)}
                    sx={{ pl: 4 }}
                  >
                    <DataCenterIcon sx={{ mr: 1 }} />
                    <ListItemText inset primary={datacenter.name} />
                    {openStates[vIdx].datacenters[dIdx].datacenter ? (
                      <ExpandLess />
                    ) : (
                      <ExpandMore />
                    )}
                  ...
                  <Collapse
                    in={openStates[vIdx].datacenters[dIdx].datacenter}
                    timeout="auto"
                    unmountOnExit
                  >
                    <List component="div" disablePadding>
                      {datacenter.clusters.map((cluster, cIdx) => (
                        <React.Fragment key={cIdx}>
                          <ListItem
                            button
                            onClick={() => toggleCluster(vIdx, dIdx, cIdx)}
                            sx={{ pl: 6 }}
                          >
                            <ClusterIcon sx={{ mr: 1 }} />
                            <ListItemText inset primary={cluster.name} />
                            {openStates[vIdx].datacenters[dIdx].clusters[cIdx]
                              .cluster ? (
                              <ExpandLess />
                            ) : (
                              <ExpandMore />
                              ...
                              {cluster.hosts.map((host, hIdx) => (
                                <React.Fragment key={hIdx}>
                                  <ListItem
                                    button
                                    onClick={() =>
                                      toggleHost(vIdx, dIdx, cIdx, hIdx)
                                    }
                                    sx={{ pl: 8 }}
                                  >
                                    <HostIcon sx={{ mr: 1 }} />
                                    ...
                                    />
                                    {openStates[vIdx].datacenters[dIdx]
                                      .clusters[cIdx].hosts[hIdx] ? (
                                      <ExpandLess />
                                    ) : (
                                      <ExpandMore />
                                    )}
